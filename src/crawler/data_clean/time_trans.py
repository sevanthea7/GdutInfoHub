...
def extract_and_convert_time(input_sentence):
    ...
    |(?:\d{4}-\d{1,2}-\d{1,2})  # YYYY-MM-DD
    |(?:\d{4}年)?
    (?:\d{1,2}月|(?:十二|十一|十|九|八|七|六|五|四|三|二|一)月)
    (?:\d{1,2}日|\d{1,2}号|(?:十|十一|十二|十三|十四|十五|十六|十七|十八|十九|二十|二十一|二十二|二十三|二十四|二十五|二十六|二十七|二十八|二十九|三十|三十一)(?:日|号))
    |(?:\d{1,2}-\d{1,2})  # MM-DD
    |(?:\d{1,2}.\d{1,2})  # MM.DD
    """
    ...
    # 对所有字典按键长度倒序排序
    maps = [relative_time_map, fuzzy_time_map, chinese_num_map]
    for item in maps:
        sorted_items = sorted(item.items(), key=lambda x: len(x[0]), reverse=True)
        sorted_dict = dict(sorted_items)
        # 更新原字典
        item.clear()
        item.update(sorted_dict)
    ...
    for keyword in time_keywords:
        ...
        # 4. 处理模糊时间（如“最近”“近一周”）
            ...
            converted_times.append(f"{start_date.strftime('%y年%m月%d日')}-{end_date.strftime('%y年%m月%d日')}")
        ...
        # 5. 处理直接日期（如“11月13日”“十二月十号”“12-05”）
            ...
            try:
                ...
                # 处理点格式（如12.05）
                    ...
                    year_part = re.search(r"(\d{4})年", keyword)  if "年" in keyword else ""
                    # 提取月份（替换“月”为分隔符）
                    ...
                    # 中文数字转阿拉伯数字
                    year = int(year_part.group(1)) if year_part else today.year
